...
struct SchemeZ2 {
    ...
    void copyTo(LOCAL SchemeZ2 &target, bool withFlips = true) const LOCAL_METHOD;
    ...
    bool isValidMerge(int i, LOCAL const SchemeZ2 &scheme) const LOCAL_METHOD;
    ...
    bool tryReduceGauss(LOCAL RandomState &state) LOCAL_METHOD;
    ...
    bool tryMerge(GLOBAL const SchemeZ2 &scheme, LOCAL RandomState &state) LOCAL_METHOD;
    bool tryProduct(GLOBAL const SchemeZ2 &scheme) LOCAL_METHOD;
    void sandwiching(LOCAL RandomState &state) LOCAL_METHOD;
    void swapBasis(LOCAL RandomState &state) LOCAL_METHOD;
    void swapSize(LOCAL RandomState &state) LOCAL_METHOD;

    void merge(GLOBAL const SchemeZ2 &scheme, int p) LOCAL_METHOD;
...
private:
    ...
    void addTriplet(int i, int j, int k, const T u, const T v, const T w) LOCAL_METHOD;
    ...
    ReduceGaussCandidate getReduceGaussCandidate(LOCAL RandomState &state) const LOCAL_METHOD;

    int findXorCombination(int uvwIndex, LOCAL int *indices, int size, LOCAL int *combination) const LOCAL_METHOD;
    void shellSort(LOCAL int *indices, LOCAL const T *values, int n) const LOCAL_METHOD;
    bool inverseMatrixZ2(int n, LOCAL int *matrix, LOCAL int *inverse) const LOCAL_METHOD;
    void invertibleMatrixZ2(int n, LOCAL int *matrix, LOCAL int *inverse, LOCAL RandomState &state) const LOCAL_METHOD;
    T matmul(const T matrix, LOCAL int *left, LOCAL int *right, int n1, int n2) const LOCAL_METHOD;
    ...
    void split(int i, int j, int k, int index, const T a1) LOCAL_METHOD;
    void reduce(int i, int index1, int index2) LOCAL_METHOD;
    void reduceGauss(int i, LOCAL int *combination, int combinationSize) LOCAL_METHOD;
    void product(GLOBAL const SchemeZ2 &scheme) LOCAL_METHOD;
...
#ifndef __METAL_VERSION__
    void saveMatrix(std::ofstream &f, std::string name, int n1, int n2, int m, LOCAL const T *matrix) const;
#endif
};

bool SchemeZ2::validateEquation(int i, int j, int k) const LOCAL_METHOD {
    int i1 = i / n[1];
    int i2 = i % n[1];
    int j1 = j / n[2];
    int j2 = j % n[2];
...
bool SchemeZ2::validate() const LOCAL_METHOD {
    if (m < 1 || m > MAX_RANK) return false;
    for (int p = 0; p < 3; p++) {
        if (n[p] < 1 || n[p] > 16 || nn[p] != n[p] * n[(p+1)%3] || nn[p] > 64) return false;
        for (int r = 0; r < m; r++)
            if (nn[p] < 64 && (uvw[p][r] >> nn[p])) return false;
    }
    bool valid = true;
...
void SchemeZ2::initializeNaive(int n1, int n2, int n3) LOCAL_METHOD {
    n[0] = n1;
    n[1] = n2;
    n[2] = n3;
...
void SchemeZ2::copyTo(LOCAL SchemeZ2 &target, bool withFlips) const LOCAL_METHOD {
    target.m = m;
...
#ifndef __METAL_VERSION__
bool SchemeZ2::read(std::istream &is, bool checkValidity) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int index = 0; index < m; index++) {
            ...
            for (int j = 0; j < nn[i]; j++) {
                if (!(is >> value) || value < 0 || value > 1) return false;
...
#ifndef __METAL_VERSION__
bool SchemeZ2::read(std::istream &is, int n1, int n2, int n3, int m, bool checkValidity) {
...
int SchemeZ2::getComplexity() const LOCAL_METHOD {
    int count = 0;
    for (int index = 0; index < m; index++) {
        for (int i = 0; i < 3; i++) {
            #ifdef __METAL_VERSION__
            count += popcount(uvw[i][index]);
#else
...
void SchemeZ2::initFlips() LOCAL_METHOD {
    for (int i = 0; i < 3; i++) {
        flips[i].clear();
...
void SchemeZ2::removeZeroes() LOCAL_METHOD {
...
void SchemeZ2::removeAt(int index) LOCAL_METHOD {
    m--;
...
void SchemeZ2::addTriplet(int i, int j, int k, const T u, const T v, const T w) LOCAL_METHOD {
    uvw[i][m] = u;
    uvw[j][m] = v;
    uvw[k][m] = w;
    m++;
...
void SchemeZ2::excludeColumn(int matrix, int column) LOCAL_METHOD {
    int n1 = n[matrix];
    int n2 = n[(matrix + 1) % 3];
...
void SchemeZ2::excludeRow(int matrix, int row) LOCAL_METHOD {
    int n1 = n[matrix];
    int n2 = n[(matrix + 1) % 3];
...
void SchemeZ2::addColumn(int matrix) LOCAL_METHOD {
    int n1 = n[matrix];
    int n2 = n[(matrix + 1) % 3];
...
void SchemeZ2::addRow(int matrix) LOCAL_METHOD {
    int n1 = n[matrix];
    int n2 = n[(matrix + 1) % 3];
...
bool SchemeZ2::isValidProject(int i, int minN) const LOCAL_METHOD {
    return n[i] - 1 >= minN && n[(i + 1) % 3] >= minN && n[(i + 2) % 3] >= minN;
}

bool SchemeZ2::isValidExtension(int i, int maxN) const LOCAL_METHOD {
...
bool SchemeZ2::isValidProduct(int i, int maxN) const LOCAL_METHOD {
    int newN[3] = {n[0], n[1], n[2]};
    newN[i] *= 2;
...
bool SchemeZ2::isValidMerge(int i, LOCAL const SchemeZ2 &scheme) const LOCAL_METHOD {
...
ReduceGaussCandidate SchemeZ2::getReduceGaussCandidate(LOCAL RandomState &state) const LOCAL_METHOD {
    int permutation[3];
...
int SchemeZ2::findXorCombination(int uvwIndex, LOCAL int *indices, int size, LOCAL int *combination) const LOCAL_METHOD {
    if (size < 3)
        return 0;
...
void SchemeZ2::shellSort(LOCAL int *indices, LOCAL const T *values, int n) const LOCAL_METHOD {
...
bool SchemeZ2::inverseMatrixZ2(int n, LOCAL int *matrix, LOCAL int *inverse) const LOCAL_METHOD {
...
void SchemeZ2::invertibleMatrixZ2(int n, LOCAL int *matrix, LOCAL int *inverse, LOCAL RandomState &state) const LOCAL_METHOD {
    do {
        randomMatrixZ2(n, matrix, state);
    } while (!inverseMatrixZ2(n, matrix, inverse));
}

T SchemeZ2::matmul(const T matrix, LOCAL int *left, LOCAL int *right, int n1, int n2) const LOCAL_METHOD {
...
void SchemeZ2::flip(int i, int j, int k, int index1, int index2, bool checkReduce) LOCAL_METHOD {
...
void SchemeZ2::plus(int i, int j, int k, int index1, int index2, int variant) LOCAL_METHOD {
...
void SchemeZ2::split(int i, int j, int k, int index, const T a1) LOCAL_METHOD {
    T a2 = uvw[i][index] ^ a1;
    uvw[i][index] = a1;
...
void SchemeZ2::reduceGauss(int i, LOCAL int *combination, int combinationSize) LOCAL_METHOD {
...
void SchemeZ2::reduce(int i, int index1, int index2) LOCAL_METHOD {
...
void SchemeZ2::project(int p, int q) LOCAL_METHOD {
    excludeRow(p, q);
...
void SchemeZ2::extend(int p) LOCAL_METHOD {
    if (p == 0) {
        addRow(0);
        addColumn(2);
...
void SchemeZ2::product(int p) LOCAL_METHOD {
...
void SchemeZ2::merge(GLOBAL const SchemeZ2 &scheme, int p) LOCAL_METHOD {
    int nNew[3];
    int nnNew[3];
    int d[3];
...
void SchemeZ2::product(GLOBAL const SchemeZ2 &scheme2) LOCAL_METHOD {
    SchemeZ2 scheme1;
...
void SchemeZ2::swapBasisRows(int i1, int i2) LOCAL_METHOD {
...
void SchemeZ2::swapBasisColumns(int j1, int j2) LOCAL_METHOD {
...
void SchemeZ2::swapSize(int p1, int p2) LOCAL_METHOD {
    if (p1 == p2)
        return;
...
bool SchemeZ2::tryFlip(LOCAL RandomState &state, bool checkReduce) LOCAL_METHOD {
    int size = flips[0].size + flips[1].size + flips[2].size;

    if (!size)
        return false;

    int index = randomWord(&state) % size;
...
bool SchemeZ2::tryPlus(LOCAL RandomState &state) LOCAL_METHOD {
...
bool SchemeZ2::trySplit(LOCAL RandomState &state) LOCAL_METHOD {
    if (m >= MAX_RANK || m >= n[0] * n[1] * n[2])
        return false;
    ...
    T a1 = T(randomWord(&state)) & (nn[i] == 64 ? ~T(0) : (T(1) << nn[i]) - 1);
    const T a2 = uvw[i][index];
    int loop = 0;

    while (loop < 5 && a1 == a2) {
        a1 = T(randomWord(&state)) & (nn[i] == 64 ? ~T(0) : (T(1) << nn[i]) - 1);
        loop++;
    }
...
bool SchemeZ2::trySplitExisted(LOCAL RandomState &state) LOCAL_METHOD {
    if (m >= MAX_RANK || m >= n[0] * n[1] * n[2])
        return false;
...
bool SchemeZ2::tryExpand(int count, LOCAL RandomState &state) LOCAL_METHOD {
...
bool SchemeZ2::tryReduce() LOCAL_METHOD {
...
bool SchemeZ2::tryReduceGauss(LOCAL RandomState &state) LOCAL_METHOD {
...
bool SchemeZ2::tryProject(LOCAL RandomState &state, int minN) LOCAL_METHOD {
    int indices[3];
    int size = 0;
...
bool SchemeZ2::tryExtend(LOCAL RandomState &state, int maxN) LOCAL_METHOD {
    int indices[3];
    int size = 0;
...
bool SchemeZ2::tryProduct(LOCAL RandomState &state, int maxN) LOCAL_METHOD {
    int indices[3];
    int size = 0;
...
bool SchemeZ2::tryMerge(GLOBAL const SchemeZ2 &scheme, LOCAL RandomState &state) LOCAL_METHOD {
    if (m + scheme.m > MAX_RANK)
        return false;
...
bool SchemeZ2::tryProduct(GLOBAL const SchemeZ2 &scheme) LOCAL_METHOD {
    if (m * scheme.m > MAX_RANK)
        return false;
...
void SchemeZ2::sandwiching(LOCAL RandomState &state) LOCAL_METHOD {
...
void SchemeZ2::swapBasis(LOCAL RandomState &state) LOCAL_METHOD {
    if (randomWord(&state) % 2) {
...
void SchemeZ2::swapSize(LOCAL RandomState &state) LOCAL_METHOD {
    int p1, p2;
...
#ifndef __METAL_VERSION__
void SchemeZ2::saveMatrix(std::ofstream &f, std::string name, int n1, int n2, int m, LOCAL const T *matrix) const {
    f << "    \"" << name << "\": [" << std::endl;
...
void SchemeZ2::save(const std::string &path) {
    if (!validate()) throw std::runtime_error("refusing to export invalid F2 scheme");
    std::ofstream f(path);
