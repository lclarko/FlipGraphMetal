...
FlipGraph::FlipGraph(int n1, int n2, int n3, int schemesCount, int blockSize, int maxIterations, int plusIterations, const std::string &path, const FlipGraphProbabilities &probabilities, int seed) {
    ...
    metalAllocate(&bestRanks, schemesCount * sizeof(int));
    metalAllocate(&flips, schemesCount * sizeof(int));
    metalAllocate(&states, schemesCount * sizeof(RandomState));

    metalDispatch("initializeRandomStatesKernel", (numBlocks) * blockSize, blockSize, states, schemesCount, seed);

}

void FlipGraph::initialize() {
    std::cout << "Start main initialization" << std::endl;
    metalDispatch("initializeSchemesKernel", (numBlocks) * blockSize, blockSize, schemes, schemesBest, bestRanks, flips, n1, n2, n3, schemesCount);
...
bool FlipGraph::initializeFromFile(std::istream &f) {
    int count;
    if (!(f >> count) || count < 1 || count > schemesCount)
        throw std::runtime_error("invalid input scheme count: expected 1..--schemes");
    ...
    metalDispatch("initializeCopyKernel", (numBlocks) * blockSize, blockSize, schemes, schemesCount, count);
...
void FlipGraph::initializeNaive() {
    ...
    metalDispatch("initializeNaiveKernel", (numBlocks) * blockSize, blockSize, schemes, schemesCount, n1, n2, n3);
...
static bool canUseCompactScheme(const SchemeInteger &scheme) {
    ...
    for (int p = 0; p < 3; p++) {
        ...
        for (int r = 0; r < scheme.m; r++) {
            const Addition &term = scheme.uvw[p][r];
            if (term.n != 9 || (term.values & ~T(511)) || (term.signs & ~T(511))) return false;
        }
    }
    return true;
}

static bool canUseCompactWalk(const SchemeInteger *schemes, const SchemeInteger *best, const int *flips,
                              int count, int blockSize, int maxIterations, int plusIterations,
                              const FlipGraphProbabilities &probabilities) {
    if (count < 1 || blockSize != 32 || maxIterations != 1000 || plusIterations != 1000000000 ||
        probabilities.reduce != 0 || probabilities.expand != 0 || probabilities.sandwiching != 0 ||
        probabilities.basis != 0 || probabilities.resize != 0) return false;
    for (int i = 0; i < count; i++)
        if (flips[i] < 0 || flips[i] >= plusIterations - maxIterations ||
            !canUseCompactScheme(schemes[i]) || !canUseCompactScheme(best[i])) return false;
    return true;
}
#endif

void FlipGraph::randomWalk() {
    const char *kernel = "randomWalkKernel";
#ifndef METAL_F2
    if (canUseCompactWalk(schemes, schemesBest, flips, schemesCount, blockSize, maxIterations, plusIterations, probabilities))
        kernel = "randomWalkCompactKernel";
#endif
    metalDispatch(kernel, (numBlocks) * blockSize, blockSize, schemes, schemesBest, bestRanks, flips, states, schemesCount, maxIterations, plusIterations, probabilities.reduce, probabilities.expand, probabilities.sandwiching, probabilities.basis, probabilities.resize > 0);

}

void FlipGraph::resize() {
    metalDispatch("resizeKernel", (numBlocks) * blockSize, blockSize, schemes, schemesBest, schemesCount, states, probabilities.resize);
...
void FlipGraph::run(int logPeriod) {
    ...
    for (int iteration = 0; metalRounds == 0 || iteration < metalRounds; iteration++) {
        randomWalk();
