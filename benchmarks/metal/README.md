...
`benchmarks/workflow/performance.py INPUT --output NEW_RECEIPT` evaluates retained observations. Version 1 evaluates paired baseline/candidate comparisons; version 2 evaluates complete native-host workflows against approved absolute latency and memory budgets. Neither mode substitutes for the other. Its input binds approved budgets, the mandatory endpoint roster, both planned looks, matched-work identities and a separate precision assessment. Unapproved budgets and insufficient evidence cannot pass. A hard resource violation remains a failure even when a practical margin is still unapproved. A slowdown inside an approved tolerance must still be reported as a slowdown.

`baseline.py --host build/metal/scheme_tool --protocol PLAN.json --output NEW_RUN` measures verified reporting, CPU-text round trips and bounded selection from generated inventories. The plan must bind approved budgets, fixtures, build settings, hardware and twelve prospective trials before acquisition. The first look acquires six trials; only an inconclusive result permits the declared second look. Complete-workflow time includes independent output verification, with native process time, verification time and peak native RSS reported separately. Input generation occurs before timing. This host-only mode does not initialize Metal or establish GPU performance.
