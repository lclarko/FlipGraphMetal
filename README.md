...
Native `--run-config` workflows provide bounded rank-reduction and fixed-rank alternative searches, retained pools, durable history and resume, corpus analysis, and direct signed circuit reduction. They draw on [Perminov's CPU implementation](https://github.com/dronperminov/ternary_flip_graph) but follow a separately specified bounded search policy. They do not reproduce its trajectories, population dynamics or optimizer objectives. See [native workflow configuration](docs/development.md#native-controlled-workflows).
...
Possible additions include further CPU pool and restart strategies, structural/type descriptors and diversity policies, meta-search, Z3 support and lifting. The bounded native workflows, projection, extension, tensor products and resizing are already implemented. Legacy behavior uses a separate policy from controlled workflows.
